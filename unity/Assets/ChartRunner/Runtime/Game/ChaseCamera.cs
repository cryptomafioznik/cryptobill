...
namespace ChartRunner.Game
{
    ...
    public class ChaseCamera : MonoBehaviour
    {
        ...
        /// <summary>
        /// Доля высоты кадра под героя — ИЗМЕРЕНА ПО ВАЛИДИРОВАННОЙ ВЕРСИИ: в браузерной
        /// игре байк с райдером занимает 7.2 % высоты (67 px из 932 по снятому кадру).
        ///
        /// Было 0.125 — моё число, назначенное «чтобы героя было видно». Оно вдвое крупнее
        /// оригинала, и цена этого измерена: обзор вперёд падал до 0.88 с при бюджете
        /// реакции 0.80 с, то есть игрок узнавал о препятствии почти в тот момент, когда
        /// уже надо действовать. Живой вердикт: «при такой скорости я вряд ли пройду
        /// какие-то препятствия» — это не про скорость байка, а про то, что впереди
        /// ничего не видно.
        ///
        /// Прежний вывод «в портрете 430×932 герой ≥12 % и обзор 1.5 с несовместимы» был
        /// арифметически верен и практически бесполезен: несовместимость создавали мои же
        /// 12 %, а игра, которую пользователь принимал годами, выбрала 7 %.
        /// </summary>
